...
class EthereumDeployer {
    ...
    constructor(provider, offChainStorage, networkId, smartContractTemplate, tokenArtifact, erc20Artifact) {
        ...
        if (tokenArtifact.networks[networkId]?.address) {
            ...
            // Initialization done for later set of the contract address
            tokenArtifact.networks[networkId] = {};
        }

        if (erc20Artifact === undefined) erc20Artifact = devErc20Artifact;
        this.erc20Artifact = erc20Artifact;

        if (erc20Artifact.networks[networkId]?.address) {
            ...
            // Initialization done for later set of the contract address
            erc20Artifact.networks[networkId] = {};
        }
    }
    ...
    async deploySmartContractsReserved() {
        ...
        if (this.mediaSC.objects !== undefined) {
            for (const objectKey of Object.keys(this.mediaSC.objects)) {
                ...
                const data = JSON.parse(objectValue.uri);
                console.debug("Object data", data);
                const metadata = data.metadata;
                if (metadata && metadata["dc:isAlreadyRegistered"] === "true") {
                    console.debug("Object", data.identifier, "already registered");
                    continue;
                }
