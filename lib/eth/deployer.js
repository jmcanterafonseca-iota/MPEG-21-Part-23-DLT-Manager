...
class EthereumDeployer {
  ...
  async reserveTokens(num) {
    console.log(`Reserving tokens...(modified)`);
    const res = await this.token.methods
      .reserveIds(num)
      .send({ from: this.mainAddress, gas: this.gas });
  ...
  async finalDeploy(deonticsIds, objectsIds) {
    ...
    console.log("setting th provided");
    this.contract.setProvider(this.provider);

    console.log("deployting contract with addresss" + this.token.options.address);
    console.log("conentn media sc" + this.mediaSC.contentURI)
  ...
  async deploySmartContractsReserved() {
    ...
    for (const deonticKey of Object.keys(this.mediaSC.deontics)) {
      ...
      console.info(`deontic recevier: ${this.mediaSC.parties[deonticValue.receiver]}: uri ${uri} and $published: ${deonticValue.uri}`)
    }
    const objectsIds = [];
    if (this.mediaSC.objects !== undefined) {
      for (const objectKey of Object.keys(this.mediaSC.objects)) {
        ...
        console.info(`object receiver: ${this.mediaSC.parties[objectValue.receiver]}: uri ${uri} and $published: ${objectValue.uri}`)
