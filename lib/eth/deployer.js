...
class EthereumDeployer {
  constructor(provider, offChainStorage, networkId, tokenArtifact, erc20Artifact) {
    ...
    this.erc20Token = new this.web3.eth.Contract(
      erc20Artifact.abi,
      erc20Artifact.networks[networkId].address
    );
  ...
  async finalDeploy(deonticsIds, objectsIds) {
    ...
    const finalIncomePercentageList = [incomePercentagesList[0]];
    for (let i = 1; i < incomePercentagesList.length; i++) {
      finalIncomePercentageList.push(new this.web3.utils.BN(Math.floor(incomePercentagesList[i] * 100)));
    }
    ...
    const contractCreationResult = await this.contract
      ...
      await contractCreationResult.methods.setERC20Token(this.erc20Artifact['networks'][this.networkId]['address']).send({
        from: this.mainAddress,
        gas: this.gas,
      });
