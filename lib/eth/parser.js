...
class EthereumParser {
  ...
  async parseSmartContract() {
    ...
    this.mediaContract = await this.offChainStorage.retrieve(contractUri);
    ...
    for (const id of deonticsTokensIds) {
      ...
      deontics.push(await this.offChainStorage.retrieve(tokenUri));
    ...
    for (const id of objectsTokensIds) {
      const tokenUri = await this.token.methods.tokenURI(id).call();
      objects.push(await this.offChainStorage.retrieve(tokenUri));
    }
